...
using ndn::tlv::nfd::FaceStatus;
using ndn::nfd::RibRegisterCommand;
using ndn::nfd::RibUnregisterCommand;
using ndn::nfd::StrategyChoiceSetCommand;
using ndn::nfd::StrategyChoiceUnsetCommand;
using ndn::nfd::ControlParameters;
using ndn::makeEmptyBlock;
...
public:
  ConfigClient()
  {
    m_session = std::to_string(ndn::random::generateSecureWord32());
...
private:
  void processFaceQueryRes(const Interest& interest, const Data& data)
  {
    ...
    Block::element_const_iterator val = responseBlock.find(FaceStatus);
    while (val != responseBlock.elements_end())
    {
      class FaceStatus status(*val);
      ...
      val = responseBlock.find(FaceStatus);
    }
  ...
  void onConfigStartRes(const Interest& interest, const Data& data)
  {
    std::cout << data << std::endl;
    // this is anchor
    ndn::security::v2::Certificate anchor(data.getContent().blockFromValue());
  ...
  {
    std::cout << data << std::endl;
    ndn::security::SafeBag safebag(data.getContent().blockFromValue());
    ndn::security::v2::Certificate cert(safebag.getCertificate());
    const char* passwd = "1234";
    try {
      m_keyChain.importSafeBag(safebag, passwd, strlen(passwd));
    }
    catch (const ndn::security::v2::KeyChain::Error& e) {
...
{
  nfd::tools::config::Program program;
